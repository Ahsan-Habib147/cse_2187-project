...
class student
{
	int idnum;
	char name[50];
	int Physics, Chemistry, Mathematics, MTE, Humanities;
	double per;
	string grade;
	void calculate();	
public:
...
{
	per=(Physics+Chemistry+Mathematics+MTE+Humanities)/5.0;
	if(per>=90)
		grade="A+";
	else if(per>=80)
		grade="A";
...
void student::getdata()
{
	...
	cin>>Mathematics;
	cout<<"\nEnter student's MTE grade: ";
	cin>>MTE;
	cout<<"\nEnter student's Humanities grade: ";
	cin>>Humanities;
	calculate();
}
...
void student::show_tabular() const
{
	cout<<idnum<<setw(6)<<" "<<name<<setw(10)<<Physics<<setw(4)<<Chemistry<<setw(4)<<Mathematics<<setw(4)
		<<MTE<<setw(4)<<Humanities<<setw(8)<<per<<setw(6)<<grade<<endl;
}
